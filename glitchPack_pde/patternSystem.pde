class PatternSystem {
  ...
  void updateZ() {
    zoff = zoff + rateFlow*map(cos(map(millis()%(beatInterval), 0, beatInterval, PI, TWO_PI)), -1, 1, 0.5, 2);
    zoff2 = zoff2 + rateFlow2*map(cos(map(millis()%(beatInterval), 0, beatInterval, PI, TWO_PI)), -1, 1, 0.5, 2);
  }
